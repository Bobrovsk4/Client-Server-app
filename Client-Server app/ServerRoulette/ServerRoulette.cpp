...
public:
	static void bet(player& p1, player& p2) {
		...
		if (player2_bet == -1) {
			...
			spin(p1, p2, player1_bet, player2_bet);
		}
	}

	static void spin(player& p1, player& p2, int bet1, int bet2) {

		std::string spinning = "Spinning roulette...\n";
		send(p1.connection, spinning.c_str(), spinning.length(), NULL);
		send(p2.connection, spinning.c_str(), spinning.length(), NULL);
		std::this_thread::sleep_for(std::chrono::milliseconds(500));

		int result = 1 + rand() % 2;

		if (result == 1) {
			std::string winner1 = p1.name + " won!\n";
			send(p1.connection, winner1.c_str(), winner1.length(), NULL);
			send(p2.connection, winner1.c_str(), winner1.length(), NULL);
			p1.balance += (bet1 + bet2);
		}
		else {
			std::string winner2 = p2.name + " won!\n";
			send(p1.connection, winner2.c_str(), winner2.length(), NULL);
			send(p2.connection, winner2.c_str(), winner2.length(), NULL);
...
int main() {
	...
	while (true) {
		if (counter < 2) {
			...
			recv(Connections[1], name2, 50, NULL);
